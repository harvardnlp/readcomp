...
function loadData(tensorData, training)
  local tensor_table = {}
  local tensor_current = {}
  local tensor_length = tensorData:size(1)
  for i = 1,tensor_length do
    if tensorData[i] == 0 then
      tensor_table[#tensor_table + 1] = torch.LongTensor(tensor_current)
      tensor_current = {}
    else
      tensor_current[#tensor_current + 1] = tensorData[i]
    end
  end
  ...
  if training then -- load ivocab, vocab and unigram word frequency
    d.ivocab = {}
    d.vocab = {}
    d.wordfreq = {}
...
local trainset   = loadData(data.train, true)
local validset   = loadData(data.valid, false)
local controlset = loadData(data.control, false)
local testset    = loadData(data.test, false)
