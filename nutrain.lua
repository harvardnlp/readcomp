...
function test_model(saved_model_file, dump_name, tensor_data, tensor_post, tensor_ner,
  ...
  for i = 1, ntestbatches do
    local tests_con, tests_ans, tests_ans_ind, tests_lineno = loadData(tensor_data,
      tensor_post, tensor_ner, tensor_sid, tensor_sentence, tensor_speech, tensor_extr,
      tensor_location, tensor_choices, false, all_batches[i])
    brnn_loc_extractor:setStuff(tensor_data, tensor_location, all_batches[i])
...
function build_model()
  ...
  if not lm then
    ...
    brnn_loc_extractor = nn.SeqBRNNAtLoc(opt.query_idx)
    U = Yd:clone():add(brnn_loc_extractor):add(nn.Unsqueeze(3)) -- batch x (2 * hiddensize) x 1
...
function train(params, grad_params, epoch)
  ...
  for ir = 1,nbatches do
    ...
    brnn_loc_extractor:setStuff(data.train_data, data.train_location, all_batches[randind[ir]])
...
function validate(ntrial, epoch)
  ...
  for i = 1, nvalbatches do
    ...
    brnn_loc_extractor:setStuff(data.valid_data, data.valid_location, all_batches[i])
